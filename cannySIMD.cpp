...
{		
	...
	float* atan2_val = static_cast<float*>(std::aligned_alloc(32, 8 * sizeof(float)));
	...
	{		
        ...
        for (; x <= width - 8; x += 8) 
		{
			...
			__m256 dir = _mm256_load_ps(atan2_val); 
...
float* double_threshold(float* input, int height, int width, float low_ratio = 0.1f, float high_ratio = 0.3f) 
{
	...
	for( ; i < totalPixel; i++)
	{
		...
		{
			thresholded[i] = 255.0f;
		}
		else 
		{
			thresholded[i] = 128.0f;  // 不確定邊緣
		}
	}
